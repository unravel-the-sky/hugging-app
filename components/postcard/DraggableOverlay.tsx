...
//
// One component for both text and sticker overlays. Owns the gesture stack
// (pan + pinch + rotate, all simultaneous) and a tap that selects / edits.
// Committed values live in parent state; shared values drive the live
// transform and write back on gesture end so they survive re-renders (and are
// correct at capture time for view-shot).
...
export default function DraggableOverlay({
  ...
  // Keep chrome (dashed border + delete) a constant on-screen size.
          ...
          <Animated.View style={boxStyle}>
            <View style={selected ? styles.selectedPad : styles.pad}>
              {overlay.kind === "text" ? (
                ...
                <Ionicons
                  name={overlay.icon}
                  size={overlay.size ?? STICKER_BASE_SIZE}
                  color={overlay.color}
                />
              )}
